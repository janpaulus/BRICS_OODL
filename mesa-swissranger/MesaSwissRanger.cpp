...
MesaSwissRanger::MesaSwissRanger() {
  ...
  // Bouml preserved body end 00032171
}

MesaSwissRanger::~MesaSwissRanger() {
  // Bouml preserved body begin 000321F1
  Errors error;
  this->close(error);
  // Bouml preserved body end 000321F1
}

bool MesaSwissRanger::open(Errors& error) {
  // Bouml preserved body begin 00033DF1
...
bool MesaSwissRanger::close(Errors& error) {
  // Bouml preserved body begin 00032271
  int returnValue = -3;
  ...
  // Bouml preserved body end 00032271
}

/**
 * 
 */
bool MesaSwissRanger::setConfiguration(const ToFCameraConfiguration& configuration, Errors& error) {
  // Bouml preserved body begin 000322F1
  if (!this->open(error)) {
    return false;
  }
  ...
  // Bouml preserved body end 000322F1
}

bool MesaSwissRanger::getConfiguration(ToFCameraConfiguration& configuration, Errors& error) {
  // Bouml preserved body begin 00032371
  if (!this->open(error)) {
    return false;
  }
 ...
 // BOOST_LOG_SEV(lg, trace) << "Vendor: " << configuration.vendor << " Product: " <<productID << " SN: " << configuration.serialNumber ;
  ...
  // Bouml preserved body end 00032371
}

bool MesaSwissRanger::getData(ToFCameraData& data, Errors& error) {
  // Bouml preserved body begin 000323F1
  if (!this->open(error)) {
    return false;
  }
  ...
  // Bouml preserved body end 000323F1
}

bool MesaSwissRanger::resetDevice(Errors& error) {
  // Bouml preserved body begin 00032471
