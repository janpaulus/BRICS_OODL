/*!
 * \file SickS300.cc
 * \brief Definition of the class SickS300
 *
...
namespace SickToolbox {
   ...
   */
  SickS300::SickS300(const std::string sick_device_path) : SickLIDAR< SickS300BufferMonitor, SickS300Message >(),
  _sick_device_path(sick_device_path),
  _curr_session_baud(SICK_BAUD_UNKNOWN),
  _desired_session_baud(SICK_BAUD_UNKNOWN),
  _sick_type(SICK_S300_TYPE_UNKNOWN),
  ...
  _sick_values_subrange_stop_index(0) {
    ...
    memset(&_sick_operating_status, 0, sizeof (sick_s300_operating_status_t));
    memset(&_sick_software_status, 0, sizeof (sick_s300_software_status_t));
    memset(&_sick_restart_status, 0, sizeof (sick_s300_restart_status_t));
    memset(&_sick_pollution_status, 0, sizeof (sick_s300_pollution_status_t));
    memset(&_sick_signal_status, 0, sizeof (sick_s300_signal_status_t));
    memset(&_sick_field_status, 0, sizeof (sick_s300_field_status_t));
    memset(&_sick_baud_status, 0, sizeof (sick_s300_baud_status_t));
    memset(&_sick_device_config, 0, sizeof (sick_s300_device_config_t));
  ...
  SickS300::~SickS300() {
    ...
    /* Catch an I/O exception */ catch (SickIOException &sick_io_exception) {
      std::cerr << sick_io_exception.what() << std::endl;
    }
    /* Catch anything else */ catch (...) {
      std::cerr << "SickS300::~SickS300: Unknown exception!" << std::endl;
    }

  }

  /**
   * \brief Attempts to initialize the Sick S300 and then sets communication at
   *        at the given baud rate.
   * \param desired_baud_rate Desired session baud rate
   */
  void SickS300::Initialize(const sick_s300_baud_t desired_baud_rate)
  throw ( SickConfigException, SickTimeoutException, SickIOException, SickThreadException) {
    ...
    try {
      ...
      std::cout << std::endl << "\t*** Attempting to initialize the Sick S300..." << std::endl << std::flush;
      ...
      /* Assume a timeout is due to a misconfigured terminal baud */ catch (SickTimeoutException &sick_timeout) {

        /* Check whether to do an autodetect */
        sick_s300_baud_t default_baud = _baudToSickBaud(DEFAULT_SICK_S300_BAUD);
        std::cout << "\tFailed to set requested baud rate..." << std::endl << std::flush;
        std::cout << "\tAttempting to detect S300 baud rate..." << std::endl << std::flush;
        if ((default_baud != SICK_BAUD_9600) && _testSickBaud(SICK_BAUD_9600)) {
          std::cout << "\t\tDetected S300 baud @ " << SickBaudToString(SICK_BAUD_9600) << "!" << std::endl;
        } else if ((default_baud != SICK_BAUD_19200) && _testSickBaud(SICK_BAUD_19200)) {
          std::cout << "\t\tDetected S300 baud @ " << SickBaudToString(SICK_BAUD_19200) << "!" << std::endl;
        } else if ((default_baud != SICK_BAUD_38400) && _testSickBaud(SICK_BAUD_38400)) {
          std::cout << "\t\tDetected S300 baud @ " << SickBaudToString(SICK_BAUD_38400) << "!" << std::endl;
        } else if ((default_baud) != SICK_BAUD_500K && _testSickBaud(SICK_BAUD_500K)) {
          std::cout << "\t\tDetected S300 baud @ " << SickBaudToString(SICK_BAUD_500K) << "!" << std::endl;
        } else {
          _stopListening();
          throw SickIOException("SickS300::Initialize: failed to detect baud rate!");
        }
        std::cout << std::flush;
      ...
      /* Catch anything else */ catch (...) {
        std::cerr << "SickS300::Initialize: Unknown exception!" << std::endl;
        throw;
      }
    ...
    /* Handle a config exception */ catch (SickConfigException &sick_config_exception) {
      std::cerr << sick_config_exception.what() << std::endl;
      throw;
    }
    /* Handle a timeout exception */ catch (SickTimeoutException &sick_timeout_exception) {
    ...
    /* Handle any thread exceptions */ catch (SickThreadException &sick_thread_exception) {
    ...
    std::cout << "\t*** Init. complete: Sick S300 is online and ready!" << std::endl;
//    std::cout << "\tSick Type: " << SickTypeToString(GetSickType()) << std::endl;
 ...
 //   std::cout << "\tMeasuring Mode: " << SickMeasuringModeToString(GetSickMeasuringMode()) << std::endl;
  ...
  /**
   * \brief Uninitializes the S300 by putting it in a mode where it stops streaming data,
   *        and returns it to the default baud rate (specified in the header).
   */
  void SickS300::Uninitialize() throw ( SickConfigException, SickTimeoutException, SickIOException, SickThreadException) {

    if (_sick_initialized) {

      std::cout << std::endl << "\t*** Attempting to uninitialize the Sick S300..." << std::endl;
      ...
      try {
        ...
        _setSessionBaud(_baudToSickBaud(DEFAULT_SICK_S300_BAUD));
        ...
        std::cout << "\t*** Uninit. complete - Sick S300 is now offline!" << std::endl << std::flush;

      }
      /* Handle any config exceptions */ catch (SickConfigException &sick_config_exception) {
      ...
      /* Handle anything else */ catch (...) {
        std::cerr << "SickS300::Unintialize: Unknown exception!!!" << std::endl;
        throw;
      }
  ...
  /**
   * \brief Gets the Sick S300 device path
   * \return The device path as a std::string
   */
  std::string SickS300::GetSickDevicePath() const {
    return _sick_device_path;
  }

  /**
   * \brief Gets the Sick S300 type
   * \return The device type
   */
  sick_s300_type_t SickS300::GetSickType() const throw ( SickConfigException) {

    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickS300::GetSickType: Sick S300 is not initialized!");
    }

    /* Return the Sick S300 type */
    return _sick_type;
   ...
   * \return Scan angle of the device (sick_s300_scan_angle_t) 
   */
  double SickS300::GetSickScanAngle() const throw ( SickConfigException) {

    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickS300::GetSickScanAngle: Sick S300 is not initialized!");
    }
   ...
   * \return Angular resolution of the Sick S300 (sick_s300_scan_resolution_t) 
   */
  double SickS300::GetSickScanResolution() const throw ( SickConfigException) {

    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickS300::GetSickScanResolution: Sick S300 is not initialized!");
    }
  ...
  //   */
  //  void SickS300::SetSickMeasuringUnits( const sick_s300_measuring_units_t sick_units )
  //    throw( SickConfigException, SickTimeoutException, SickIOException, SickThreadException ) {
  //
  //    /* Ensure the device is initialized */
  //    if (!_sick_initialized) {
  //      throw SickConfigException("SickS300::SetSickMeasuringUnits: Sick S300 is not initialized!");
  //    }
  //
  //    /* Ensure a valid units type was given */
  //    if (!_validSickMeasuringUnits(sick_units)) {
  //      throw SickConfigException("SickS300::SetSickMeasuringUnits: Undefined measurement units!");
  //    }
  //
  ...
  //      sick_s300_device_config_t sick_device_config;
  //
  ...
  //      catch(...) {
  //      	std::cerr << "SickS300::SetSickMeasuringUnits: Unknown exception!!!" << std::endl;
  //      	throw;
  //      }
  //
  //    }
  //    else {
  //      std::cerr << "\tSickS300::SetSickMeasuringUnits - Device is already configured w/ these units. (skipping write)" << std::endl;
  //    }
  //
  //  }

  /**
   * \brief Gets the current Sick S300 measuring units
   * \return Measuring units (sick_s300_measuring_units_t)
   */
  sick_s300_measuring_units_t SickS300::GetSickMeasuringUnits() const throw ( SickConfigException) {

    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickS300::GetSickMeasuringUnits: Sick S300 is not initialized!");
    }

    /* Return the measurement units */
    return (sick_s300_measuring_units_t) _sick_operating_status.sick_measuring_units;

  }

  //  /**
  //   * \brief Sets the Sick S300 sensitivity level
  //   * \param sick_sensitivity Desired sensitivity level
  //   */
  //  void SickS300::SetSickSensitivity( const sick_s300_sensitivity_t sick_sensitivity )
  //    throw( SickConfigException, SickTimeoutException, SickIOException, SickThreadException ) {
  //
  //    /* Ensure the device is initialized */
  //    if (!_sick_initialized) {
  //      throw SickConfigException("SickS300::SetSickSensitivity: Sick S300 is not initialized!");
  //    }
  //
  //    /* Ensure the command is supported by the given Sick model */
  //    if (!_isSickS300211() && !_isSickS300221() && !_isSickS300291()) {
  //      throw SickConfigException("SickS300::SetSickSensitivity: This command is not supported by this Sick model!");
  //    }
  //
  //    /* Ensure this is a valid sick sensitivity value*/
  //    if (!_validSickSensitivity(sick_sensitivity)) {
  //      throw SickConfigException("SickS300::SetSickSensitivity: Undefined sensitivity level!");
  //    }
  //
  //    /* Make sure the write is necessary */
  ...
  //      /* Handle anything else */
  //      catch(...) {
  //      	std::cerr << "SickS300::SetSickSensitivity: Unknown exception!!!" << std::endl;
  //      	throw;
  //      }
  //
  //    }
  //    else {
  //      std::cerr << "\tSickS300::SetSickSensitivity - Sick is already operating at this sensitivity level! (skipping write)" << std::endl;
  //    }
  //
  //  }
  ...
  //   * \param sick_sensitivity Desired sensitivity level
  //   */
  //  void SickS300::SetSickPeakThreshold( const sick_s300_peak_threshold_t sick_peak_threshold )
  //    throw( SickConfigException, SickTimeoutException, SickIOException, SickThreadException ) {
  //
  //    /* Ensure the device is initialized */
  //    if (!_sick_initialized) {
  //      throw SickConfigException("SickS300::SetSickPeakThreshold: Sick S300 is not initialized!");
  //    }
  //
  //    /* Ensure the command is supported by the given Sick model */
  //    if (!_isSickS300200() && !_isSickS300220()) {
  //      throw SickConfigException("SickS300::SetSickPeakThreshold: This command is not supported by this Sick model!");
  //    }
  //
  //    /* Ensure this is a valid sick sensitivity value*/
  //    if (!_validSickPeakThreshold(sick_peak_threshold)) {
  //      throw SickConfigException("SickS300::SetSickPeakThreshold: Undefined peak threshold!");
  //    }
  //
  //    /* Make sure the write is necessary */
  ...
  //      /* Handle anything else */
  //      catch(...) {
  //      	std::cerr << "SickS300::SetSickPeakThreshold: Unknown exception!!!" << std::endl;
  //      	throw;
  //      }
  //
  //    }
  //    else {
  //      std::cerr << "\tSickS300::SetSickPeakThreshold - Sick is already operating w/ given threshold! (skipping write)" << std::endl;
  //    }
  //
  //  }
  //
  //  /**
  //   * \brief Gets the current Sick S300 sensitivity level
  //   * \return Sensitivity level (sick_s300_sensitivity_t)
  //   */
  //  sick_s300_sensitivity_t SickS300::GetSickSensitivity( ) const throw( SickConfigException ) {
  //
  //    /* Ensure the device is initialized */
  //    if (!_sick_initialized) {
  //      throw SickConfigException("SickS300::GetSickSensitivity: Sick S300 is not initialized!");
  //    }
  //
  ...
  //    return (sick_s300_sensitivity_t)_sick_device_config.sick_peak_threshold;  //If the device is 211/221/291 then this value is sensitivity
  //
  //  }
  ...
  //   * \return Sensitivity level (sick_s300_sensitivity_t)
  //   */
  //  sick_s300_peak_threshold_t SickS300::GetSickPeakThreshold( ) const throw( SickConfigException ) {
  //
  //    /* Ensure the device is initialized */
  //    if (!_sick_initialized) {
  //      throw SickConfigException("SickS300::GetSickPeakThreshold: Sick S300 is not initialized!");
  //    }
  //
  //    /* Make sure sensitivity is something that is defined for this model */
  //    if(!_isSickS300200() && !_isSickS300220()) {
  ...
  //
  //    /* If its supported than return the actual value */
  //    return (sick_s300_peak_threshold_t)_sick_device_config.sick_peak_threshold;  //If the device is 211/221/291 then this value is sensitivity
  //
  //  }
  ...
  //   */
  //  void SickS300::SetSickMeasuringMode( const sick_s300_measuring_mode_t sick_measuring_mode )
  //    throw( SickConfigException, SickTimeoutException, SickIOException, SickThreadException ) {
  //
  //    /* Ensure the device is initialized */
  //    if (!_sick_initialized) {
  ...
  //      throw SickConfigException("SickS300::SetSickMeasuringMode: Undefined measuring mode!");
  //    }
  //
  //    /* Make sure the write is necessary */
  ...
  //      /* Handle anything else */
  //      catch(...) {
  //      	std::cerr << "SickS300::SetSickMeasuringMode: Unknown exception!!!" << std::endl;
  //      	throw;
  //      }
  //
  //    }
  //    else {
  //      std::cerr << "\tSickS300::SetSickMeasuringMode - Sick is already operating w/ this measuring mode! (skipping write)" << std::endl;
  //    }
  //
  //  }
  //
  //  /**
  //   * \brief Gets the current Sick S300 measuring mode
  //   * \return Measuring mode (sick_s300_measuring_mode_t)
  //   */
  //  sick_s300_measuring_mode_t SickS300::GetSickMeasuringMode( ) const throw( SickConfigException ) {
  //
  //    /* Ensure the device is initialized */
  //    if (!_sick_initialized) {
  //      throw SickConfigException("SickS300::GetSickMeasuringMode: Sick S300 is not initialized!");
  //    }
  //
  //    /* Return the determined measuring mode */
  //    return (sick_s300_measuring_mode_t)_sick_operating_status.sick_measuring_mode;
  //
  //  }
  //
  //  /**
  //   * \brief Gets the current Sick S300 operating mode
  //   * \return Operating mode (sick_s300_operating_mode_t)
  //   */
  //  sick_s300_operating_mode_t SickS300::GetSickOperatingMode( ) const throw( SickConfigException ) {
  //
  //    /* Ensure the device is initialized */
  //    if (!_sick_initialized) {
  //      throw SickConfigException("SickS300::GetSickScanAngle: Sick S300 is not initialized!");
  ...
  //    return (sick_s300_operating_mode_t)_sick_operating_status.sick_operating_mode;
  //
  //  }
  //
  //  /**
  //   * \brief Sets the availability level of the device
  //   * \param sick_availability_level Desired availability of the Sick S300
  //   */
  //  void SickS300::SetSickAvailability( const uint8_t sick_availability_flags )
  //    throw( SickConfigException, SickTimeoutException, SickIOException, SickThreadException ) {
  //
  //    /* Ensure the device is initialized */
  //    if (!_sick_initialized) {
  //      throw SickConfigException("SickS300::SetSickAvailabilityFlags: Sick S300 is not initialized!");
  //    }
  //
  //    /* Ensure this is a valid sick sensitivity value*/
  //    if ( sick_availability_flags > 7 ) {
  //      throw SickConfigException("SickS300::SetSickAvailabilityFlags: Invalid availability!");
  //    }
  //
  //    /* Setup a local copy of the config */
  //    sick_s300_device_config_t sick_device_config;
  ...
  //      /* Handle anything else */
  //      catch(...) {
  //      	std::cerr << "SickS300::SetSickAvailabilityFlags: Unknown exception!!!" << std::endl;
  //      	throw;
  //      }
  //
  //    }
  //    else {
  //      std::cerr << "\tSickS300::SetSickAvailability - Device is already operating w/ given availability. (skipping write)" << std::endl;
  //    }
  //
  //  }
  //
  //  /**
  //   * \brief Gets the current Sick S300 availability level flags
  //   * \return Sick S300 Availability level flags
  //   */
  //  uint8_t SickS300::GetSickAvailability( ) const throw( SickConfigException ) {
  //
  //    /* Ensure the device is initialized */
  //    if (!_sick_initialized) {
  //      throw SickConfigException("SickS300::GetSickAvailabilityFlags: Sick S300 is not initialized!");
  //    }
  //
  ...
  /*!
   * \brief Sets the variant of the Sick S300 (scan angle and scan resolution)
   * \param scan_angle The desired scan angle of the Sick S300
   * \param scan_resolution The desired angular resolution of the Sick S300
   */
  void SickS300::SetSickVariant(const sick_s300_scan_angle_t scan_angle, const sick_s300_scan_resolution_t scan_resolution)
  throw ( SickConfigException, SickTimeoutException, SickIOException, SickThreadException) {

    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickS300::SetSickVariant: Sick S300 is not initialized!");
    }

    /* A sanity check to make sure that the command is supported */
    if (_sick_type == SICK_S300_TYPE_211_S14 || _sick_type == SICK_S300_TYPE_221_S14 || _sick_type == SICK_S300_TYPE_291_S14) {
      throw SickConfigException("SickS300::SetSickVariant: Command not supported on this model!");
    }

    /* Ensure the given scan angle is defined */
    if (!_validSickScanAngle(scan_angle)) {
      throw SickConfigException("SickS300::SetSickVariant: Undefined scan angle!");
    }

    /* Ensure the given scan resolution is defined */
    if (!_validSickScanResolution(scan_resolution)) {
      throw SickConfigException("SickS300::SetSickMeasuringUnits: Undefined scan resolution!");
    }

    SickS300Message message, response;
    ...
    switch (scan_angle) {
      ...
      case SICK_SCAN_ANGLE_180:
        ...
        break;
      default:
        throw SickConfigException("SickS300::SetSickVariant: Given scan angle is invalid!");
    }
    ...
    switch (scan_resolution) {
      ...
      case SICK_SCAN_RESOLUTION_25:
        ...
        break;
      default:
        throw SickConfigException("SickS300::SetSickVariant: Given scan resolution is invalid!");
    }

    /* Build the request message */
    message.BuildMessage(DEFAULT_SICK_S300_ADDRESS, payload_buffer, 5);
    ...
    try {
      ...
      _sendMessageAndGetReply(message, response, DEFAULT_SICK_S300_MESSAGE_TIMEOUT, DEFAULT_SICK_S300_NUM_TRIES);
    ...
    /* Handle anything else */ catch (...) {
      std::cerr << "SickS300::SetSickVariant: Unknown exception!!!" << std::endl;
      throw;
    }
    ...
    if (payload_buffer[1] != 0x01) {
      throw SickConfigException("SickS300::SetSickVariant: Configuration was unsuccessful!");
    }
    ...
    _sick_operating_status.sick_scan_angle =
            sick_s300_to_host_byte_order(_sick_operating_status.sick_scan_angle);
    ...
    _sick_operating_status.sick_scan_resolution =
            sick_s300_to_host_byte_order(_sick_operating_status.sick_scan_resolution);

  }

  /**
   * \brief Returns the most recent measured values obtained by the Sick S300
   ...
   *       of the Sick S300 for this value to be populated.
   */
  void SickS300::GetSickScan(unsigned int * const measurement_values,
    ...
    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickS300::GetSickScan: Sick S300 is not initialized!");
    }

    /* Declare message objects */
    SickS300Message response;
    ...
    try {
      ...
      _recvMessage(response, DEFAULT_SICK_S300_MESSAGE_TIMEOUT);

      /* Check that our payload has the proper command byte of 0xB0 */
      if (response.GetCommandCode() != 0xB0) {
        throw SickIOException("SickS300::GetSickScan: Unexpected message!");
      }
      ...
      sick_s300_scan_profile_b0_t sick_scan_profile;

      /* Initialize the profile */
      memset(&sick_scan_profile, 0, sizeof (sick_s300_scan_profile_b0_t));
    ...
    /* Handle anything else */ catch (...) {
      std::cerr << "SickS300::GetSickScan: Unknown exception!!!" << std::endl;
      throw;
    }
  ...
  /**
   * \brief Acquires both range and reflectivity values from the Sick S300 211/221/291-S14 (S300-FAST)
   ...
   *       of the Sick S300 for this value to be populated.
   */
  void SickS300::GetSickScan(unsigned int * const range_values,
    ...
    try {
      ...
      /* Define a local scan profile object */
      sick_s300_scan_profile_c4_t sick_scan_profile;

      /* Initialize the profile */
      memset(&sick_scan_profile, 0, sizeof (sick_s300_scan_profile_c4_t));
   ...
   *       of the Sick S300 for this value to be populated.
   *
   * EXAMPLE: Using a 180/0.5 Sick S300 variant, a subrange over the scan area [0,10.5] would
   *          correspond to setting the index arguments as follows: sick_subrange_start = 1,
   *          sick_subrange_stop = 22
   */
  void SickS300::GetSickScanSubrange(const uint16_t sick_subrange_start_index,
    ...
    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickS300::GetSickScanSubrange: Sick S300 is not initialized!");
    }
    ...
    try {
      ...
      if (response.GetCommandCode() != 0xB7) {
        throw SickIOException("SickS300::GetSickScanSubrange: Unexpected message!");
      }
      ...
      /* Define a local scan profile object */
      sick_s300_scan_profile_b7_t sick_scan_profile;

      /* Initialize the profile */
      memset(&sick_scan_profile, 0, sizeof (sick_s300_scan_profile_b7_t));
    ...
    /* Handle anything else */ catch (...) {
      std::cerr << "SickS300::GetSickScanSubrange: Unknown exception!!!" << std::endl;
      throw;
    }
  ...
  /**
   * \brief Returns the most recent partial scan obtained by Sick S300
   * \param *measurement_values Destination buffer for holding the current round of measured value
   * \param &num_measurement_values Number of values stored in measurement_values
   ...
   *       Sick S300 variant to use a 0.25 degrees scan resolution.
   * 
   * NOTE: Calling this function will return either range or reflectivity measurements
   *       depending upon the current measuring mode of the device.
   *
   ...
   *       of the Sick S300 for this value to be populated.
   */
  void SickS300::GetSickPartialScan(unsigned int * const measurement_values,
          unsigned int & num_measurement_values,
    ...
    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickS300::GetSickPartialScan: Sick S300 is not initialized!");
    }
    ...
    try {
      ...
      /* Check that our payload has the proper command byte of 0xB0 */
      if (response.GetCommandCode() != 0xB0) {
        throw SickIOException("SickS300::GetSickPartialScan: Unexpected message!");
      }
    ...
    /* Handle anything else */ catch (...) {
      std::cerr << "SickS300::GetSickPartialScan: Unknown exception!!!" << std::endl;
      throw;
    }
  ...
  /**
   * \brief Returns the most recent mean measured values from the Sick S300.
   ...
   *       of the Sick S300 for this value to be populated.
   */
  void SickS300::GetSickMeanValues(const uint8_t sick_sample_size,
          unsigned int * const measurement_values,
          unsigned int & num_measurement_values,
          unsigned int * const sick_telegram_index,
          unsigned int * const sick_real_time_scan_index) throw ( SickConfigException, SickTimeoutException, SickIOException, SickThreadException) {
    ...
    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickS300::GetSickMeanValues: Sick S300 is not initialized!");
    }
    ...
    try {
      ...
      _recvMessage(response, DEFAULT_SICK_S300_MEAN_VALUES_MESSAGE_TIMEOUT);

      /* Check that our payload has the proper command byte of 0xB0 */
      if (response.GetCommandCode() != 0xB6) {
        throw SickIOException("SickS300::GetSickMeanValues: Unexpected message!");
      }
      ...
      /* Define a local scan profile object */
      sick_s300_scan_profile_b6_t sick_scan_profile;

      /* Initialize the profile */
      memset(&sick_scan_profile, 0, sizeof (sick_s300_scan_profile_b6_t));
    ...
    /* Handle anything else */ catch (...) {
      std::cerr << "SickS300::GetSickMeanValues: Unknown exception!!!" << std::endl;
      throw;
    }
   ...
   *          sick_subrange_stop = 22
   */
  void SickS300::GetSickMeanValuesSubrange(const uint8_t sick_sample_size,
    ...
    /* Ensure the device is initialized */
    if (!_sick_initialized) {
      throw SickConfigException("SickS300::GetSickMeanValuesSubrange: Sick S300 is not initialized!");
    }
    ...
    try {
      ...
      if (response.GetCommandCode() != 0xBF) {
        throw SickIOException("SickS300::GetSickMeanValuesSubrange: Unexpected message!");
      }
      ...
      /* Define a local scan profile object */
      sick_s300_scan_profile_bf_t sick_scan_profile;

      /* Initialize the profile */
      memset(&sick_scan_profile, 0, sizeof (sick_s300_scan_profile_bf_t));
    ...
    /* Handle anything else */ catch (...) {
      std::cerr << "SickS300::GetMeanValuesSubrange: Unknown exception!!!" << std::endl;
      throw;
    }
  ...
  //  /**
  //   * \brief Acquire the Sick S300 status
  //   * \return The status of the device
  //   *
  //   * NOTE: This method also updated the local view of all other information
  //   *       returned with a status request.
  //   */
  //  sick_s300_status_t SickS300::GetSickStatus( ) throw( SickConfigException, SickTimeoutException, SickIOException, SickThreadException ) {
  //
  //    /* Ensure the device is initialized */
  //    if (!_sick_initialized) {
  //      throw SickConfigException("SickS300::GetSickStatus: Sick S300 is not initialized!");
  //    }
  //
  //    try {
  //
  ...
  //    /* Catch anything else */
  //    catch(...) {
  //      std::cerr << "SickS300::GetSickStatus: Unknown exception!" << std::endl;
  //      throw;
  //    }
  //
  //    /* Return the latest Sick status */
  //    return (sick_s300_status_t)_sick_operating_status.sick_device_status;
  //  }

  //  /**
  //   * \brief Indicates whether the device is an S300 Fast
  //   */
  //  bool SickS300::IsSickS300Fast() const throw(SickConfigException) {
  //
  //    /* Ensure the device is initialized */
  //    if (!_sick_initialized) {
  //      throw SickConfigException("SickS300::IsSickS300Fast: Sick S300 is not initialized!");
  //    }
  //
  //    return (_sick_type == SICK_S300_TYPE_211_S14 ||
  //	    _sick_type == SICK_S300_TYPE_221_S14 ||
  //	    _sick_type == SICK_S300_TYPE_291_S14);
  //
  //  }

  //  /**
  //   * \brief Reset the Sick S300 active field values
  //   * NOTE: Considered successful if the S300 ready message is received.
  //   */
  //  void SickS300::ResetSick( ) throw( SickConfigException, SickTimeoutException, SickIOException, SickThreadException ) {
  //
  //    /* Ensure the device is initialized */
  //    if (!_sick_initialized) {
  //      throw SickConfigException("SickS300::ResetSick: Sick S300 is not initialized!");
  //    }
  //
  //    SickS300Message message,response;
  //    uint8_t payload[SickS300Message::MESSAGE_PAYLOAD_MAX_LENGTH] = {0};
  //
  //    /* Construct the reset command */
  //    payload[0] = 0x10; // Request field reset
  //    message.BuildMessage(DEFAULT_SICK_S300_ADDRESS,payload,1);
  //
  ...
  //      _sendMessageAndGetReply(message,response,0x91,(unsigned int)60e6,DEFAULT_SICK_S300_NUM_TRIES);
  //
  //      std::cout << "\t\tPower on message received!" << std::endl;
  //      std::cout << "\tWaiting for S300 Ready message..." << std::endl;
  //
  //      /* Set terminal baud to the detected rate to get the S300 ready message */
  //      _setTerminalBaud(_baudToSickBaud(DEFAULT_SICK_S300_BAUD));
  //
  //      /* Receive the S300 ready message after power on */
  //      _recvMessage(response,(unsigned int)30e6);
  //
  //      /* Verify the response */
  //      if(response.GetCommandCode() != 0x90) {
  // 	std::cerr << "SickS300::ResetSick: Unexpected reply! (assuming device has been reset!)"  << std::endl;
  //      } else {
  // 	std::cout << "\t\tS300 Ready message received!" << std::endl;
  //      }
  //      std::cout << std::endl;
  //
  ...
  //    /* Catch anything else */
  //    catch(...) {
  //      std::cerr << "SickS300::ResetSick: Unknown exception!!!" << std::endl;
  //      throw;
  //    }
  //
  //    std::cout << "\tRe-initialization sucessful. S300 is ready to go!" << std::endl;
  //
  //  }

  //  /**
  //   * \brief Acquire the Sick S300's status as a printable string
  //   * \return The Sick S300 status as a well-formatted string
  //   */
  //  std::string SickS300::GetSickStatusAsString( ) const {
  //
  //    std::stringstream str_stream;
  //
  //    str_stream << "\t=============== Sick S300 Status ===============" << std::endl;
  //
  ...
  //      str_stream << "\tSensor Status: " << SickStatusToString((sick_s300_status_t)_sick_operating_status.sick_device_status) << std::endl;
  ...
  //
  //  /**
  //   * \brief Acquire the Sick S300's operating params as a printable string
  //   * \return The Sick S300 operating params as a well-formatted string
  //   */
  //  std::string SickS300::GetSickSoftwareVersionAsString( ) const {
  //
  //    std::stringstream str_stream;
  //
  //    str_stream << "\t============== Sick S300 Software ==============" << std::endl;
  //
  //    if (_sick_initialized) {
  //
  ...
  //
  //  /**
  //   * \brief Acquire the Sick S300's config as a printable string
  //   * \return The Sick S300 config as a well-formatted string
  //   */
  //  std::string SickS300::GetSickConfigAsString( ) const {
  //
  //    std::stringstream str_stream;
  //
  //    str_stream<< "\t=============== Sick S300 Config ===============" << std::endl;
  //
  //    if (_sick_initialized) {
  //
  ...
  //      }
  //      else {
  //	str_stream << "\tPeak Thresh: " << SickPeakThresholdToString((sick_s300_peak_threshold_t)_sick_device_config.sick_peak_threshold) << std::endl;
  ...
  //      str_stream << "\tMeasuring Mode: " << SickMeasuringModeToString((sick_s300_measuring_mode_t)_sick_device_config.sick_measuring_mode) << std::endl;
  //      str_stream << "\tMeasuring Units: " << SickMeasuringUnitsToString((sick_s300_measuring_units_t)_sick_device_config.sick_measuring_units) << std::endl;
  ...
  //
  //  /**
  //   * \brief Prints ths status of the Sick S300 unit
  //   */
  //  void SickS300::PrintSickStatus() const {
  ...
  //   */
  //  void SickS300::PrintSickSoftwareVersion() const {
  //    std::cout << GetSickSoftwareVersionAsString() << std::endl;
  //  }
  //
  //  /**
  //   * \brief Prints out the Sick S300 configurations parameters
  //   */
  //  void SickS300::PrintSickConfig() const {
  //    std::cout << GetSickConfigAsString() << std::endl;
  //  }
  //
  //  /**
  //   * \brief Converts the Sick S300 type to a corresponding string
  //   * \param sick_type The device type
  //   * \return Sick S300 type as a string
  //   */
  //  std::string SickS300::SickTypeToString( const sick_s300_type_t sick_type ) {
  //
  //    switch(sick_type) {
  //    case SICK_S300_TYPE_200_30106:
  //      return "Sick S300 200-30106";
  //    case SICK_S300_TYPE_211_30106:
  //      return "Sick S300 211-30106";
  //    case SICK_S300_TYPE_211_30206:
  //      return "Sick S300 211-30206";
  //    case SICK_S300_TYPE_211_S07:
  //      return "Sick S300 211-S07";
  //    case SICK_S300_TYPE_211_S14:
  //      return "Sick S300 211-S14";
  //    case SICK_S300_TYPE_211_S15:
  //      return "Sick S300 211-S15";
  //    case SICK_S300_TYPE_211_S19:
  //      return "Sick S300 211-S19";
  //    case SICK_S300_TYPE_211_S20:
  //      return "Sick S300 211-S20";
  //    case SICK_S300_TYPE_220_30106:
  //      return "Sick S300 220-30106";
  //    case SICK_S300_TYPE_221_30106:
  //      return "Sick S300 221-30106";
  //    case SICK_S300_TYPE_221_30206:
  //      return "Sick S300 221-30206";
  //    case SICK_S300_TYPE_221_S07:
  //      return "Sick S300 221-S07";
  //    case SICK_S300_TYPE_221_S14:
  //      return "Sick S300 221-S14";
  //    case SICK_S300_TYPE_221_S15:
  //      return "Sick S300 221-S15";
  //    case SICK_S300_TYPE_221_S16:
  //      return "Sick S300 221-S16";
  //    case SICK_S300_TYPE_221_S19:
  //      return "Sick S300 221-S19";
  //    case SICK_S300_TYPE_221_S20:
  //      return "Sick S300 221-S20";
  //    case SICK_S300_TYPE_291_S05:
  //      return "Sick S300 291-S05";
  //    case SICK_S300_TYPE_291_S14:
  //      return "Sick S300 291-S14";
  //    case SICK_S300_TYPE_291_S15:
  //      return "Sick S300 291-S15";
  //    default:
  //      return "Unknown!";
  //    }
  //
  ...
  /**
   * \brief Converts integer to corresponding Sick S300 scan angle
   * \param scan_angle_int Scan angle (FOV) as an integer (e.g. 90,100,180)
   */
  sick_s300_scan_angle_t SickS300::IntToSickScanAngle(const int scan_angle_int) {
  ...
  /**
   * \brief Converts integer to corresponding Sick S300 scan resolution
   * \param scan_resolution_int Scan resolution as an integer (e.g. 25,50,100)
   */
  sick_s300_scan_resolution_t SickS300::IntToSickScanResolution(const int scan_resolution_int) {
  ...
  /**
   * \brief Converts double to corresponding Sick S300 scan resolution
   * \param scan_resolution_double Scan resolution as a double (e.g. 0.25,0.5,1.0)
   */
  sick_s300_scan_resolution_t SickS300::DoubleToSickScanResolution(const double scan_resolution_double) {
    return IntToSickScanResolution((const int) (scan_resolution_double * 100));
  }

  /**
   * \brief Converts Sick S300 baud to a corresponding string
   * \param baud_rate The baud rate to be represented as a string
   * \return The string representation of the baud rate
   */
  std::string SickS300::SickBaudToString(const sick_s300_baud_t baud_rate) {
  ...
  /**
   * \brief Converts integer to corresponding Sick S300 baud
   * \param baud_str Baud rate as integer (e.g. 9600,19200,38400,500000)
   */
  sick_s300_baud_t SickS300::IntToSickBaud(const int baud_int) {
  ...
  /**
   * \brief Converts string to corresponding Sick S300 baud
   * \param baud_str Baud rate as string (e.g. "9600","19200","38400","500000")
   */
  sick_s300_baud_t SickS300::StringToSickBaud(const std::string baud_str) {
  ...
  /**
   * \brief Converts the Sick S300 status code to a string
   * \param sick_status The device status
   * \return A string corresponding to the given status code
   */
  std::string SickS300::SickStatusToString(const sick_s300_status_t sick_status) {
   ...
   */
  std::string SickS300::SickMeasuringModeToString(const sick_s300_measuring_mode_t sick_measuring_mode) {
   ...
   * \return The corresponding string
   */
  std::string SickS300::SickOperatingModeToString(const sick_s300_operating_mode_t sick_operating_mode) {
  ...
  /**
   * \brief Converts Sick S300 sensitivity level to a corresponding string
   * \param sick_sensitivity Sick sensitivity level
   * \return The corresponding string
   */
  std::string SickS300::SickSensitivityToString(const sick_s300_sensitivity_t sick_sensitivity) {
  ...
  /**
   * \brief Converts Sick S300 peak threshold to a corresponding string
   * \param sick_peak_threshold Sick sensitivity level
   * \return The corresponding string
   */
  std::string SickS300::SickPeakThresholdToString(const sick_s300_peak_threshold_t sick_peak_threshold) {
  ...
  /**
   * \brief Converts the Sick S300 measurement units to a corresponding string
   * \param sick_units The measuring units
   * \return The corresponding string
   */
  std::string SickS300::SickMeasuringUnitsToString(const sick_s300_measuring_units_t sick_units) {
   ...
   */
  void SickS300::_setupConnection() throw ( SickIOException, SickThreadException) {

    try {

      /* Open the device */
      if ((_sick_fd = open(_sick_device_path.c_str(), O_RDWR | O_NOCTTY)) < 0) {
        throw SickIOException("SickS300::_setupConnection: - Unable to open serial port");
      }

      /* Backup the original term settings */
      if (tcgetattr(_sick_fd, &_old_term) < 0) {
        throw SickIOException("SickS300::_setupConnection: tcgetattr() failed!");
      }
    ...
    /* Handle unknown exceptions */ catch (...) {
      std::cerr << "SickS300::_setupConnection: Unknown exception!" << std::endl;
      throw;
    }
   ...
   */
  void SickS300::_teardownConnection() throw ( SickIOException) {
    ...
    if (tcsetattr(_sick_fd, TCSANOW, &_old_term) < 0) {
      throw SickIOException("SickS300::_teardownConnection: tcsetattr() failed!");
    }

    /* Actually close the device */
    if (close(_sick_fd) != 0) {
      throw SickIOException("SickS300::_teardownConnection: close() failed!");
    }
   ...
   */
  void SickS300::_flushTerminalBuffer() throw ( SickThreadException) {
    ...
    try {
      ...
      if (tcflush(_sick_fd, TCIOFLUSH) != 0) {
        throw SickThreadException("SickS300::_flushTerminalBuffer: tcflush() failed!");
      }
    ...
    /* Handle thread exceptions */ catch (SickThreadException &sick_thread_exception) {
      std::cerr << sick_thread_exception.what() << std::endl;
      throw;
    }
    /* A sanity check */ catch (...) {
      std::cerr << "SickS300::_flushTerminalBuffer: Unknown exception!" << std::endl;
      throw;
    }
   ...
   * \param &send_message The message to be sent to the Sick S300 unit
   * \param &recv_message The expected message reply from the Sick S300
   ...
   */
  void SickS300::_sendMessageAndGetReply(const SickS300Message &send_message,
          SickS300Message &recv_message,
    ...
    /* Handle a timeout! */ catch (SickTimeoutException &sick_timeout) {
    ...
    /* Handle write buffer exceptions */ catch (SickIOException &sick_io_error) {
      std::cerr << sick_io_error.what() << std::endl;
      throw;
    }
    /* A safety net */ catch (...) {
      std::cerr << "SickS300::_sendMessageAndGetReply: Unknown exception!!!" << std::endl;
      throw;
    }
   ...
   * \param timeout_value The epoch to wait before considering a sent frame lost (in usecs)
   * \param num_tries The number of times to send the message in the event the S300 fails to reply
   */
  void SickS300::_sendMessageAndGetReply(const SickS300Message &send_message,
          SickS300Message &recv_message,
    ...
    try {
      ...
      SickLIDAR< SickS300BufferMonitor, SickS300Message >::_sendMessageAndGetReply(send_message, recv_message, &reply_code, 1, DEFAULT_SICK_S300_BYTE_INTERVAL, timeout_value, num_tries);
   ...
   */
  void SickS300::_setSessionBaud(const sick_s300_baud_t baud_rate) throw ( SickIOException, SickThreadException, SickTimeoutException) {
    ...
    if (baud_rate == SICK_BAUD_UNKNOWN) {
      throw SickIOException("SickS300::_setSessionBaud: Undefined baud rate!");
    }
    ...
    message.BuildMessage(DEFAULT_SICK_S300_ADDRESS, payload, 2);
    ...
    /* Catch a timeout */ catch (SickTimeoutException &sick_timeout_exception) {
      std::cerr << sick_timeout_exception.what() << std::endl;
      throw;
    }
    /* Catch any I/O exceptions */ catch (SickIOException &sick_io_exception) {
      std::cerr << sick_io_exception.what() << std::endl;
      throw;
    }
    /* Catch any thread exceptions */ catch (SickThreadException &sick_thread_exception) {
      std::cerr << sick_thread_exception.what() << std::endl;
      throw;
    }
    /* Catch anything else */ catch (...) {
      std::cerr << "SickS300::_getSickErrors: Unknown exception!!!" << std::endl;
      throw;
    }
  ...
  /**
   * \brief Attempts to detect whether the S300 is operating at the given baud rate
   * \param baud_rate The baud rate to use when "pinging" the Sick S300
   */
  bool SickS300::_testSickBaud(const sick_s300_baud_t baud_rate) throw ( SickIOException, SickThreadException) {

    try {

      /* Another sanity check */
      if (baud_rate == SICK_BAUD_UNKNOWN) {
        throw SickIOException("SickS300::_testBaudRate: Undefined baud rate!");
      }
      ...
      /* Catch a timeout exception */ catch (SickTimeoutException &sick_timeout_exception) {
        /* This means that the current baud rate timed out! */
        return false;
      }
      /* Catch anything else and throw it away */ catch (...) {
        std::cerr << "SickS300::_testBaudRate: Unknown exception!" << std::endl;
        throw;
      }

    }
    /* Handle any IO exceptions */ catch (SickIOException &sick_io_exception) {
      std::cerr << sick_io_exception.what() << std::endl;
      throw;
    }
    /* Handle thread exceptions */ catch (SickThreadException &sick_thread_exception) {
   ...
   */
  void SickS300::_setTerminalBaud(const sick_s300_baud_t baud_rate) throw ( SickIOException, SickThreadException) {
    ...
    try {
      ...
      if (baud_rate == SICK_BAUD_500K) {
        ...
        if (ioctl(_sick_fd, TIOCGSERIAL, &serial) < 0) {
          throw SickIOException("SickS300::_setTerminalBaud: ioctl() failed!");
        }
...
#else
        throw SickIOException("SickS300::_setTerminalBaud - 500K baud is only supported under Linux!");
#endif
        ...
        if (ioctl(_sick_fd, TIOCGSERIAL, &serial) < 0) {
          std::cerr << "SickS300::_setTermSpeed: ioctl() failed while trying to get serial port info!" << std::endl;
        ...
        if (ioctl(_sick_fd, TIOCSSERIAL, &serial) < 0) {
          std::cerr << "SickS300::_setTerminalBaud: ioctl() failed while trying to set serial port info!" << std::endl;
          std::cerr << "\tNOTE: This is normal when connected via USB!" << std::endl;
        }
      ...
      if (tcgetattr(_sick_fd, &term) < 0) {
        throw SickIOException("SickS300::_setTerminalBaud: Unable to get device attributes!");
      }
      ...
      switch (baud_rate) {
        ...
        default:
          throw SickIOException("SickS300::_setTerminalBaud: Unknown baud rate!");
      }

      /* Attempt to set the device attributes */
      if (tcsetattr(_sick_fd, TCSAFLUSH, &term) < 0) {
        throw SickIOException("SickS300::_setTerminalBaud: Unable to set device attributes!");
      }
    ...
    /* Catch an IO exception */ catch (SickIOException sick_io_exception) {
      std::cerr << sick_io_exception.what() << std::endl;
      throw;
    }
    /* Catch an IO exception */ catch (SickThreadException sick_thread_exception) {
      std::cerr << sick_thread_exception.what() << std::endl;
      throw;
    }
    /* A sanity check */ catch (...) {
      std::cerr << "SickS300::_setTerminalBaud: Unknown exception!!!" << std::endl;
      throw;
    }
   ...
   */
  void SickS300::_getSickType() throw ( SickTimeoutException, SickIOException, SickThreadException) {
    ...
    /* Get the S300 type */
    payload_buffer[0] = 0x3A; //Command to request S300 type

    /* Build the message */
    message.BuildMessage(DEFAULT_SICK_S300_ADDRESS, payload_buffer, 1);
    ...
    /* Catch any timeout exceptions */ catch (SickTimeoutException &sick_timeout_exception) {
      std::cerr << sick_timeout_exception.what() << std::endl;
      throw;
    }
    /* Catch any I/O exceptions */ catch (SickIOException &sick_io_exception) {
    ...
    /* Catch anything else */ catch (...) {
      std::cerr << "SickS300::_getSickType: Unknown exception!!!" << std::endl;
      throw;
    }
    ...
    /* Set the Sick S300 type in the driver */
    if (type_string.find("S300200;30106") != std::string::npos) {
      _sick_type = SICK_S300_TYPE_200_30106;
    } else if (type_string.find("S300211;30106") != std::string::npos) {
      _sick_type = SICK_S300_TYPE_211_30106;
    } else if (type_string.find("S300211;30206") != std::string::npos) {
      _sick_type = SICK_S300_TYPE_211_30206;
    } else if (type_string.find("S300211;S07") != std::string::npos) {
      _sick_type = SICK_S300_TYPE_211_S07;
    } else if (type_string.find("S300211;S14") != std::string::npos) {
      _sick_type = SICK_S300_TYPE_211_S14;
    } else if (type_string.find("S300211;S15") != std::string::npos) {
      _sick_type = SICK_S300_TYPE_211_S15;
    } else if (type_string.find("S300211;S19") != std::string::npos) {
      _sick_type = SICK_S300_TYPE_211_S19;
    } else if (type_string.find("S300211;S20") != std::string::npos) {
      _sick_type = SICK_S300_TYPE_211_S20;
    } else if (type_string.find("S300220;30106") != std::string::npos) {
      _sick_type = SICK_S300_TYPE_220_30106;
    } else if (type_string.find("S300221;30106") != std::string::npos) {
      _sick_type = SICK_S300_TYPE_221_30106;
    } else if (type_string.find("S300221;30206") != std::string::npos) {
      _sick_type = SICK_S300_TYPE_221_30206;
    } else if (type_string.find("S300221;S07") != std::string::npos) {
      _sick_type = SICK_S300_TYPE_221_S07;
    } else if (type_string.find("S300221;S14") != std::string::npos) {
      _sick_type = SICK_S300_TYPE_221_S14;
    } else if (type_string.find("S300221;S15") != std::string::npos) {
      _sick_type = SICK_S300_TYPE_221_S15;
    } else if (type_string.find("S300221;S16") != std::string::npos) {
      _sick_type = SICK_S300_TYPE_221_S16;
    } else if (type_string.find("S300221;S19") != std::string::npos) {
      _sick_type = SICK_S300_TYPE_221_S19;
    } else if (type_string.find("S300221;S20") != std::string::npos) {
      _sick_type = SICK_S300_TYPE_221_S20;
    } else if (type_string.find("S300291;S05") != std::string::npos) {
      _sick_type = SICK_S300_TYPE_291_S05;
    } else if (type_string.find("S300291;S14") != std::string::npos) {
      _sick_type = SICK_S300_TYPE_291_S14;
    } else if (type_string.find("S300291;S15") != std::string::npos) {
      _sick_type = SICK_S300_TYPE_291_S15;
    } else {
      _sick_type = SICK_S300_TYPE_UNKNOWN;
    }
  ...
  /**
   * \brief Acquires (and buffers) the current Sick S300 configuration from the device
   */
  void SickS300::_getSickConfig() throw ( SickTimeoutException, SickIOException, SickThreadException) {
    ...
    /* Catch anything else */ catch (...) {
      std::cerr << "SickS300::_getSickConfig: Unknown exception!!!" << std::endl;
      throw;
    }
   ...
   * \param &sick_device_config The desired Sick S300 configuration
   */
  void SickS300::_setSickConfig(const sick_s300_device_config_t &sick_device_config)
  throw ( SickConfigException, SickTimeoutException, SickIOException, SickThreadException) {
    ...
    try {
      ...
      /* Set Block A */
      uint16_t temp_buffer = host_to_sick_s300_byte_order(sick_device_config.sick_blanking);
      memcpy(&payload_buffer[1], &temp_buffer, 2);

      /* Set Block B */
      payload_buffer[3] = sick_device_config.sick_stop_threshold; // NOTE: This value will be 0 for S300 211/221/291
      ...
      /* Set Block A3 */
      temp_buffer = host_to_sick_s300_byte_order(sick_device_config.sick_fields_b_c_restart_times);
      memcpy(&payload_buffer[31], &temp_buffer, 2);

      /* Set Block A4 */
      temp_buffer = host_to_sick_s300_byte_order(sick_device_config.sick_dazzling_multiple_evaluation);
      memcpy(&payload_buffer[33], &temp_buffer, 2);

      /* Populate the message container */
      message.BuildMessage(DEFAULT_SICK_S300_ADDRESS, payload_buffer, 35);

      /* Send the status request and get a reply */
      _sendMessageAndGetReply(message, response, DEFAULT_SICK_S300_CONFIG_MESSAGE_TIMEOUT, DEFAULT_SICK_S300_NUM_TRIES);
      ...
      if (payload_buffer[1] != 0x01) {
        throw SickConfigException("SickS300::_setSickConfig: Configuration failed!");
      }
    ...
    /* Catch any config exceptions */ catch (SickConfigException &sick_config_exception) {
      std::cerr << sick_config_exception.what() << std::endl;
      throw;
    }
    /* Catch any I/O exceptions */ catch (SickIOException &sick_io_exception) {
    ...
    /* Catch anything else */ catch (...) {
      std::cerr << "SickS300::_setSickConfig: Unknown exception!" << std::endl;
      throw;
    }
  ...
  /**
   * \brief Obtains any error codes from the Sick S300
   */
  void SickS300::_getSickErrors(unsigned int * const num_sick_errors, uint8_t * const error_type_buffer,
    ...
    /* The command to request S300 status */
  ...
  /**
   * \brief Acquires (and buffers) the status of the Sick S300
   */
  void SickS300::_getSickStatus() throw ( SickTimeoutException, SickIOException, SickThreadException) {
    ...
    /* Catch anything else */ catch (...) {
      std::cerr << "SickS300::_getSickStatus: Unknown exception!" << std::endl;
      throw;
    }
    ...
    /*
     * Extract the current Sick S300 operating config
     */

    /* Buffer the Sick S300 operating mode */
    ...
    _sick_operating_status.sick_num_motor_revs = sick_s300_to_host_byte_order(_sick_operating_status.sick_num_motor_revs);
    ...
    /*
     * Extract the current Sick S300 software config
     */
    ...
    /*
     * Extract the Sick S300 restart config
     */
    ...
    _sick_restart_status.sick_restart_time =
            sick_s300_to_host_byte_order(_sick_restart_status.sick_restart_time);

    /*
     * Extract the Sick S300 pollution status
     */
    ...
    for (unsigned int i = 0, k = 19; i < 8; i++, k += 2) {
      ...
      _sick_pollution_status.sick_pollution_vals[i] =
              sick_s300_to_host_byte_order(_sick_pollution_status.sick_pollution_vals[i]);
    }
    ...
    for (unsigned int i = 0, k = 35; i < 4; i++, k += 2) {
      ...
      _sick_pollution_status.sick_reference_pollution_vals[i] =
              sick_s300_to_host_byte_order(_sick_pollution_status.sick_reference_pollution_vals[i]);
    }
    ...
    for (unsigned int i = 0, k = 43; i < 8; i++, k += 2) {
      ...
      _sick_pollution_status.sick_pollution_calibration_vals[i] =
              sick_s300_to_host_byte_order(_sick_pollution_status.sick_pollution_calibration_vals[i]);
    }
    ...
    for (unsigned int i = 0, k = 59; i < 4; i++, k += 2) {
      ...
      _sick_pollution_status.sick_reference_pollution_calibration_vals[i] =
              sick_s300_to_host_byte_order(_sick_pollution_status.sick_reference_pollution_calibration_vals[i]);
    }

    /*
     * Extract the Sick S300 signal config 
     */
    ...
    _sick_signal_status.sick_reference_scale_1_dark_100 =
            sick_s300_to_host_byte_order(_sick_signal_status.sick_reference_scale_1_dark_100);
    ...
    _sick_signal_status.sick_reference_scale_2_dark_100 =
            sick_s300_to_host_byte_order(_sick_signal_status.sick_reference_scale_2_dark_100);
    ...
    _sick_signal_status.sick_reference_scale_1_dark_66 =
            sick_s300_to_host_byte_order(_sick_signal_status.sick_reference_scale_1_dark_66);
    ...
    _sick_signal_status.sick_reference_scale_2_dark_66 =
            sick_s300_to_host_byte_order(_sick_signal_status.sick_reference_scale_2_dark_66);
    ...
    _sick_signal_status.sick_signal_amplitude =
            sick_s300_to_host_byte_order(_sick_signal_status.sick_signal_amplitude);
    ...
    _sick_signal_status.sick_current_angle =
            sick_s300_to_host_byte_order(_sick_signal_status.sick_current_angle);
    ...
    _sick_signal_status.sick_peak_threshold =
            sick_s300_to_host_byte_order(_sick_signal_status.sick_peak_threshold);
    ...
    _sick_signal_status.sick_angle_of_measurement =
            sick_s300_to_host_byte_order(_sick_signal_status.sick_angle_of_measurement);
    ...
    _sick_signal_status.sick_signal_amplitude_calibration_val =
            sick_s300_to_host_byte_order(_sick_signal_status.sick_signal_amplitude_calibration_val);
    ...
    _sick_signal_status.sick_stop_threshold_target_value =
            sick_s300_to_host_byte_order(_sick_signal_status.sick_stop_threshold_target_value);
    ...
    _sick_signal_status.sick_peak_threshold_target_value =
            sick_s300_to_host_byte_order(_sick_signal_status.sick_peak_threshold_target_value);
    ...
    _sick_signal_status.sick_stop_threshold_actual_value =
            sick_s300_to_host_byte_order(_sick_signal_status.sick_stop_threshold_actual_value);
    ...
    _sick_signal_status.sick_peak_threshold_actual_value =
            sick_s300_to_host_byte_order(_sick_signal_status.sick_peak_threshold_actual_value);
    ...
    _sick_signal_status.sick_reference_target_single_measured_vals =
            sick_s300_to_host_byte_order(_sick_signal_status.sick_reference_target_single_measured_vals);
    ...
    _sick_signal_status.sick_reference_target_mean_measured_vals =
            sick_s300_to_host_byte_order(_sick_signal_status.sick_reference_target_mean_measured_vals);


    /*
     * Extract the Sick S300 field config
     */
    ...
    /*
     * Extract the Sick S300 baud config
     */
    ...
    _sick_baud_status.sick_baud_rate =
            sick_s300_to_host_byte_order(_sick_baud_status.sick_baud_rate);
    ...
    //  sick_s300_to_host_byte_order(_sick_status_data.sick_calibration_counter_0_value_1);
    ...
    //  sick_s300_to_host_byte_order(_sick_status_data.sick_calibration_counter_0_value_2);
    ...
    //  sick_s300_to_host_byte_order(_sick_status_data.sick_calibration_counter_1_value_1);
    ...
    //  sick_s300_to_host_byte_order(_sick_status_data.sick_calibration_counter_1_value_2);

    /* Buffer M0 value counter 0 */
    //memcpy(&_sick_status_data.sick_counter_0_M0,&payload_buffer[147],2);
    //_sick_status_data.sick_counter_0_M0 = sick_s300_to_host_byte_order(_sick_status_data.sick_counter_0_M0);

    /* Buffer M0 value counter 1 */
    //memcpy(&_sick_status_data.sick_counter_1_M0,&payload_buffer[149],2);
    //_sick_status_data.sick_counter_1_M0 = sick_s300_to_host_byte_order(_sick_status_data.sick_counter_1_M0);

    /* Buffer calibration interval */
    //memcpy(&_sick_status_data.sick_calibration_interval,&payload_buffer[151],2);
    //_sick_status_data.sick_calibration_interval = sick_s300_to_host_byte_order(_sick_status_data.sick_calibration_interval);
   ...
   */
  void SickS300::_setSickOpModeInstallation()
  throw ( SickConfigException, SickIOException, SickThreadException, SickTimeoutException) {
    ...
    uint8_t sick_password[9] = DEFAULT_SICK_S300_PASSWORD;
    ...
    if (_sick_operating_status.sick_operating_mode != SICK_OP_MODE_INSTALLATION) {
      ...
      /* Catch anything else */ catch (...) {
        std::cerr << "SickS300::_setSickOpModeInstallation: Unknown exception!!!" << std::endl;
        throw;
      }
   ...
   */
  void SickS300::_setSickOpModeDiagnostic()
  throw ( SickConfigException, SickIOException, SickThreadException, SickTimeoutException) {
   ...
   */
  void SickS300::_setSickOpModeMonitorRequestValues()
  throw ( SickConfigException, SickIOException, SickThreadException, SickTimeoutException) {
    ...
    if (_sick_operating_status.sick_operating_mode != SICK_OP_MODE_MONITOR_REQUEST_VALUES) {
      ...
      /* Catch anything else */ catch (...) {
        std::cerr << "SickS300::_setSickOpModeMonitorRequestValues: Unknown exception!!!" << std::endl;
        throw;
      }
   ...
   */
  void SickS300::_setSickOpModeMonitorStreamValues()
  throw ( SickConfigException, SickIOException, SickThreadException, SickTimeoutException) {
    ...
    if (_sick_operating_status.sick_operating_mode != SICK_OP_MODE_MONITOR_STREAM_VALUES) {
      ...
      /* Catch anything else */ catch (...) {
        std::cerr << "SickS300::_setSickOpModeMonitorStreamValues: Unknown exception!!!" << std::endl;
        throw;
      }
   ...
   */
  void SickS300::_setSickOpModeMonitorStreamRangeAndReflectivity()
  throw ( SickConfigException, SickIOException, SickThreadException, SickTimeoutException) {

    /* A sanity check to make sure that the command is supported */
    if (_sick_type != SICK_S300_TYPE_211_S14 && _sick_type != SICK_S300_TYPE_221_S14 && _sick_type != SICK_S300_TYPE_291_S14) {
      throw SickConfigException("SickS300::_setSickOpModeMonitorStreamRangeAndReflectivity: Mode not supported by this model!");
    }
    ...
    if (_sick_operating_status.sick_operating_mode != SICK_OP_MODE_MONITOR_STREAM_RANGE_AND_REFLECT) {
      ...
      /* Catch anything else */ catch (...) {
        std::cerr << "SickS300::_setSickOpModeStreamRangeAndReflectivity: Unknown exception!!!" << std::endl;
        throw;
      }
   ...
   */
  void SickS300::_setSickOpModeMonitorStreamValuesFromPartialScan()
  throw ( SickConfigException, SickIOException, SickThreadException, SickTimeoutException) {
    ...
    if (_sick_operating_status.sick_operating_mode != SICK_OP_MODE_MONITOR_STREAM_VALUES_FROM_PARTIAL_SCAN) {
      ...
      /* Catch anything else */ catch (...) {
        std::cerr << "SickS300::_setSickOpModeStreamValuesFromPartialScan: Unknown exception!!!" << std::endl;
        throw;
      }
   ...
   */
  void SickS300::_setSickOpModeMonitorStreamMeanValues(const uint8_t sample_size)
  throw ( SickConfigException, SickIOException, SickThreadException, SickTimeoutException) {
    ...
    if (_sick_operating_status.sick_operating_mode != SICK_OP_MODE_MONITOR_STREAM_MEAN_VALUES ||
      ...
      if (sample_size < 2 || sample_size > 250) {
        throw SickConfigException("SickS300::_setSickOpModeMonitorStreamMeanValues: Invalid sample size!");
      }
      ...
      /* Catch anything else */ catch (...) {
        std::cerr << "SickS300::_setSickOpModeStreamRangeFromPartialScan: Unknown exception!!!" << std::endl;
        throw;
      }
   ...
   */
  void SickS300::_setSickOpModeMonitorStreamValuesSubrange(const uint16_t subrange_start_index, const uint16_t subrange_stop_index)
  throw ( SickConfigException, SickIOException, SickThreadException, SickTimeoutException) {
    ...
    if (_sick_operating_status.sick_operating_mode != SICK_OP_MODE_MONITOR_STREAM_VALUES_SUBRANGE ||
      ...
      if (subrange_start_index > subrange_stop_index || subrange_start_index == 0 || subrange_stop_index > max_subrange_stop_index) {
        throw SickConfigException("SickS300::_setSickOpMonitorStreamValuesSubrange: Invalid subregion bounds!");
      }
      ...
      temp_buffer = host_to_sick_s300_byte_order(subrange_start_index);
      memcpy(mode_params, &temp_buffer, 2);

      /* Assign the subrange stop index */
      temp_buffer = host_to_sick_s300_byte_order(subrange_stop_index);
   ...
   * \param subrange_stop_index The stopping index of the desired subrange
   */
  void SickS300::_setSickOpModeMonitorStreamMeanValuesSubrange(const uint16_t sample_size, const uint16_t subrange_start_index, const uint16_t subrange_stop_index)
  throw ( SickConfigException, SickIOException, SickThreadException, SickTimeoutException) {
    ...
    if (_sick_operating_status.sick_operating_mode != SICK_OP_MODE_MONITOR_STREAM_MEAN_VALUES_SUBRANGE ||
      ...
      if (sample_size < 2 || sample_size > 250) {
        throw SickConfigException("SickS300::_setSickOpModeMonitorStreamMeanValuesSubrange: Invalid sample size!");
      }
      ...
      /* Ensure the subregion is properly defined for the given variant */
      if (subrange_start_index > subrange_stop_index || subrange_start_index == 0 || subrange_stop_index > max_subrange_stop_index) {
        throw SickConfigException("SickS300::_setSickOpMonitorStreamMeanValuesSubrange: Invalid subregion bounds!");
      }
  ...
  /**
   * \brief Attempts to switch the operating mode of the Sick S300
   * \param sick_mode The desired operating mode
   * \param mode_params Additional parameters required to set the new operating mode
   */
  void SickS300::_switchSickOperatingMode(const uint8_t sick_mode, const uint8_t * const mode_params)
  throw ( SickConfigException, SickIOException, SickThreadException, SickTimeoutException) {
    ...
    switch (sick_mode) {
      ...
      case SICK_OP_MODE_INSTALLATION:
        ...
        if (mode_params == NULL) {
          throw SickConfigException("SickS300::_switchSickOperatingMode - Requested mode requires parameters!");
        }

        memcpy(&payload_buffer[2], mode_params, 8); //Copy password
        message.BuildMessage(DEFAULT_SICK_S300_ADDRESS, payload_buffer, 10);
        break;

      case SICK_OP_MODE_DIAGNOSTIC:
        message.BuildMessage(DEFAULT_SICK_S300_ADDRESS, payload_buffer, 2);
        break;
      ...
      case SICK_OP_MODE_MONITOR_STREAM_MEAN_VALUES:
        ...
        message.BuildMessage(DEFAULT_SICK_S300_ADDRESS, payload_buffer, 3);
        break;
      ...
      case SICK_OP_MODE_MONITOR_STREAM_VALUES_SUBRANGE:
        ...
        message.BuildMessage(DEFAULT_SICK_S300_ADDRESS, payload_buffer, 6);
        break;
      ...
      case SICK_OP_MODE_MONITOR_STREAM_MEAN_VALUES_SUBRANGE:
        ...
        message.BuildMessage(DEFAULT_SICK_S300_ADDRESS, payload_buffer, 7);
        break;
      ...
      case SICK_OP_MODE_MONITOR_STREAM_RANGE_AND_REFLECT_FROM_PARTIAL_SCAN:
        ...
        message.BuildMessage(DEFAULT_SICK_S300_ADDRESS, payload_buffer, num_partial_scans * 4 + 4);
        break;
      ...
      default:
        throw SickConfigException("SickS300::_switchSickOperatingMode: Unrecognized operating mode!");
    }

    try {

      /* Attempt to send the message and get the reply */
      _sendMessageAndGetReply(message, response, DEFAULT_SICK_S300_SWITCH_MODE_TIMEOUT, DEFAULT_SICK_S300_NUM_TRIES);
    ...
    /* Catch anything else */ catch (...) {
      std::cerr << "SickS300::_switchSickOperatingMode: Unknown exception!!!" << std::endl;
      throw;
    }
    ...
    if (payload_buffer[1] != 0x00) {
      throw SickConfigException("SickS300::_switchSickOperatingMode: configuration request failed!");
    }
   ...
   */
  void SickS300::_parseSickScanProfileB0(const uint8_t * const src_buffer, sick_s300_scan_profile_b0_t &sick_scan_profile) const {
   ...
   */
  void SickS300::_parseSickScanProfileB6(const uint8_t * const src_buffer, sick_s300_scan_profile_b6_t &sick_scan_profile) const {
   ...
   * \param &sick_scan_profile The returned scan profile for the current round of mean measurements
   */
  void SickS300::_parseSickScanProfileB7(const uint8_t * const src_buffer, sick_s300_scan_profile_b7_t &sick_scan_profile) const {

    /* Read Block A, Sick S300 measured value subrange start index */
    sick_scan_profile.sick_subrange_start_index = src_buffer[0] + 256 * src_buffer[1];

    /* Read Block B, Sick S300 measured value subrange stop index */
   ...
   * \param &sick_scan_profile The returned scan profile for the current round of mean measurements
   */
  void SickS300::_parseSickScanProfileBF(const uint8_t * const src_buffer, sick_s300_scan_profile_bf_t &sick_scan_profile) const {

    /* Read Block A, the sample size used in computing the mean return */
    sick_scan_profile.sick_sample_size = src_buffer[0];

    /* Read Block B, Sick S300 measured value subrange start index */
    sick_scan_profile.sick_subrange_start_index = src_buffer[1] + 256 * src_buffer[2];

    /* Read Block C, Sick S300 measured value subrange stop index */
   ...
   * \param &sick_scan_profile The returned scan profile for the current round of measurements
   */
  void SickS300::_parseSickScanProfileC4(const uint8_t * const src_buffer, sick_s300_scan_profile_c4_t &sick_scan_profile) const {
   ...
   */
  void SickS300::_parseSickConfigProfile(const uint8_t * const src_buffer, sick_s300_device_config_t &sick_device_config) const {

    /* Buffer Block A */
    memcpy(&sick_device_config.sick_blanking, &src_buffer[0], 2);
    sick_device_config.sick_blanking = sick_s300_to_host_byte_order(sick_device_config.sick_blanking);

    /* Buffer Block B */
    sick_device_config.sick_peak_threshold = src_buffer[3]; // NOTE: This value represent sensitivity for S300 211/221/291
    sick_device_config.sick_stop_threshold = src_buffer[2]; // NOTE: This value will be 0 for S300 211/221/291
    ...
    sick_device_config.sick_fields_b_c_restart_times =
            sick_s300_to_host_byte_order(sick_device_config.sick_fields_b_c_restart_times);
    ...
    sick_device_config.sick_dazzling_multiple_evaluation =
            sick_s300_to_host_byte_order(sick_device_config.sick_dazzling_multiple_evaluation);
   ...
   */
  void SickS300::_extractSickMeasurementValues(const uint8_t * const byte_sequence, const uint16_t num_measurements, uint16_t * const measured_values,
   ...
   */
  bool SickS300::_validSickMeasuringUnits(const sick_s300_measuring_units_t sick_units) const {
  ...
  /**
   * \brief Indicates whether the Sick is an S300 200
   * \return True if the device is a Sick S300 200, False otherwise
   */
  bool SickS300::_isSickS300200() const {
  ...
  /**
   * \brief Indicates whether the Sick is an S300 211
   * \return True if the device is a Sick S300 211, False otherwise
   */
  bool SickS300::_isSickS300211() const {
  ...
  /**
   * \brief Indicates whether the Sick is an S300 220
   * \return True if the device is a Sick S300 220, False otherwise
   */
  bool SickS300::_isSickS300220() const {
  ...
  /**
   * \brief Indicates whether the Sick is an S300 221
   * \return True if the device is a Sick S300 221, False otherwise
   */
  bool SickS300::_isSickS300221() const {
  ...
  /**
   * \brief Indicates whether the Sick is an S300 291
   * \return True if the device is a Sick S300 291, False otherwise
   */
  bool SickS300::_isSickS300291() const {
   ...
   */
  bool SickS300::_isSickUnknown() const {
    return _sick_type == SICK_S300_TYPE_UNKNOWN;
  }
   ...
   */
  bool SickS300::_validSickScanAngle(const sick_s300_scan_angle_t sick_scan_angle) const {
   ...
   */
  bool SickS300::_validSickScanResolution(const sick_s300_scan_resolution_t sick_scan_resolution) const {
   ...
   */
  bool SickS300::_validSickSensitivity(const sick_s300_sensitivity_t sick_sensitivity) const {
   ...
   * \param sick_peak_threshold Peak threshold definition for Sick S300
   */
  bool SickS300::_validSickPeakThreshold(const sick_s300_peak_threshold_t sick_peak_threshold) const {
   ...
   */
  bool SickS300::_validSickMeasuringMode(const sick_s300_measuring_mode_t sick_measuring_mode) const {
   ...
   * \param baud_rate The baud rate to be converted to a Sick S300 baud
   * \return The Sick S300 equivalent of the given baud rate
   */
  sick_s300_baud_t SickS300::_baudToSickBaud(const int baud_rate) const {
   ...
   * \param availability_flags The availability level of the Sick S300
   * \return The corresponding string
   */
  std::string SickS300::_sickAvailabilityToString(const uint8_t availability_flags) const {
   ...
   * \return The corresponding string
   */
  std::string SickS300::_sickRestartToString(const uint8_t restart_code) const {
    ...
    switch (restart_code) {
      ...
      case 0x04:
        ...
        break;
      case 0x05:
        restart_str += "S300 operates as a slave, restart after set time";
        break;
      case 0x06:
        restart_str += "S300 operates as a slave, immediate restart";
        break;
      default:
        restart_str += "Unknown!";
    }
  ...
  /**
   * \brief Converts Sick S300 temporary field code to a corresponding string
   * \param temp_field_code The temporary field code
   * \return The corresponding string
   */
  std::string SickS300::_sickTemporaryFieldToString(const uint8_t temp_field_code) const {
  ...
  /**
   * \brief Converts Sick S300 subtractive fields code to a corresponding string
   * \param subt_field_code The subtractive fields code
   * \return The corresponding string
   */
  std::string SickS300::_sickSubtractiveFieldsToString(const uint8_t subt_field_code) const {
  ...
  /*!
   * \brief Converts Sick S300 contour function code to a corresponding string
   * \param contour_function_code The subtractive fields code
   * \return The corresponding string
   */
  std::string SickS300::_sickContourFunctionToString(const uint8_t contour_function_code) const {
  ...
  /**
   * \brief Converts the Sick S300 variant to a corresponding string
   * \param sick_variant The Sick S300 variant
   * \return The corresponding string
   */
  std::string SickS300::_sickVariantToString(const unsigned int sick_variant) const {

    /* Return the correct string */
    if (sick_variant == SICK_S300_VARIANT_TYPE_6) {
      return "Standard device (S300,type 6)";
    } else if (sick_variant == SICK_S300_VARIANT_SPECIAL) {
      return "Special device (S300211-/221-S19/-S20)";
    } else {
      return "Unknown";
    }
