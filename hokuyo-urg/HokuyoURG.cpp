...
bool HokuyoURG::open(Errors& error) {
  ...
  switch (this->config->baud) {
    case BAUD_9600:
      desired_baud = 9600;
      LOG(lg, trace) << "using 9600 baut to comunicate to Hokuyo URG";
      break;
    case BAUD_19200:
      desired_baud = 19200;
      LOG(lg, trace) << "using 19200 baut to comunicate to Hokuyo URG";
      break;
    case BAUD_38400:
      desired_baud = 38400;
      LOG(lg, trace) << "using 38400 baut to comunicate to Hokuyo URG";
      break;
    case BAUD_115200:
      desired_baud = 115200;
      LOG(lg, trace) << "using 115200 baut to comunicate to Hokuyo URG";
      break;
    case BAUD_500K:
      desired_baud = 500000;
      LOG(lg, trace) << "using 500K baut to comunicate to Hokuyo URG";
      break;
    case BAUD_UNKNOWN:
      desired_baud = 0;
      break;
  ...
  try {
    ...
    LOG(lg, trace) << "connection to Hokuyo URG initilized";
  } catch (...) {
...
bool HokuyoURG::close(Errors& error) {
  // Bouml preserved body begin 0002DCF1
    ...
    try {
      ...
      LOG(lg, trace) << "connection to Hokuyo URG closed";
  ...
  *(this->config) = configuration;
  ...
  if (!this->open(error)) {
   ...
   try{
     ...
     LOG(lg, trace) << "configuration set to Hokuyo URG";
...
bool HokuyoURG::getConfiguration(LaserScannerConfiguration& configuration, Errors& error) {
  ...
  parameters = this->hokuyo.parameter();
     ...
     LOG(lg, trace) << "configuration read from Hokuyo URG";
...
bool HokuyoURG::getData(LaserScannerData& data, Errors& error) {
 ...
 //   intput_ranges.assign(541, 0);
    ...
    LOG(lg, trace) << "range scan received from Hokuyo URG";
...
bool HokuyoURG::getData(LaserScannerDataWithIntensities& data, Errors& error) {
  ...
  //  this->hokuyo->getScan(intput_ranges, intput_range_angles, intput_intensity);
    ...
    LOG(lg, trace) << "range and intensity scan received from Hokuyo URG";
...
bool HokuyoURG::resetDevice(Errors& error) {
  ...
  LOG(lg, trace) << "Hokuyo URG reseted";
  return true;
